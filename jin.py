...
async def mute(ctx, member: discord.Member=None, time=None, *, reason=None):
	if not member:
		...
		await ctx.send("mention mute duration")
	else:
		if not reason:
  	...
  	await member.remove_roles(Muted)
    	unmute_embed = discord.Embed(title="Unmuted user", description=f'{ctx.author.mention} muted to {member.mention} for {reason} is over after {time}")
